...
Bob 1.8+ 的阿里云百炼图片 OCR 插件，公开包支持按量付费和 Token Plan。插件直接使用你在 Bob 设置中保存的 API Key 请求百炼，不经过第三方服务器。
...
插件固定非流式，不提供文字坐标或 Bounding Box。Token Plan 模型会在发请求前校验；阿里云禁止 Coding Plan 用于自定义应用，因此公开 manifest 和运行时都不提供该线路。
